...
class RonManager(BaseAgent):
    ...
    def _calculate_captain_score(
        ...
        """
        Captain suitability score for a player. Delegates to the shared
        ml/captain_scoring.py module so the exact live armband logic is
        replayable in the backtest harness (ron_clanker-wxlx). Signals:
        position multiplier, ceiling/variance bonus, fixture difficulty,
        home advantage, availability, minutes reliability.
        """
        from ml.captain_scoring import calculate_captain_score
        return calculate_captain_score(
            player, gameweek, self.db if hasattr(self, 'db') else None
        )
